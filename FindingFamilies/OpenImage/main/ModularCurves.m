    ...
    */  
    ...
    repeat
        ...
        */
        ...
        // We now check that enough terms of the q-expansions are computed to impose the vanishing.
        if  &or[ mult[i]/widths[i] ge M`prec/N : i in [1..#mult] ] then yyy:=prec+15; continue; end if;
        // We now construct a Q-basis FF of V.
        C:=[];
        for f in M`F do        
            c:=&cat[ &cat[Eltseq( Coefficient(f[i],j*(N div widths[i])) ) : j in [0..mult[i]-1]] : i in [1..#cusps] ];
        ...
        C:=ChangeRing(Matrix(C),Integers());
        B:=Basis(Kernel(C));
        if #B eq 0 then M`F0:=[];  break; end if;  // if V=0, we are done.
        d:=#M`F;
        FF:=[];
        for b in B do
            FF:= FF cat [ [&+[b[j]*M`F[j][i]: j in [1..d]]: i in [1..#cusps]] ];
...
function FindModelOfXG(M, prec : compute_all:=true, G0:=1, ratpoint:=false, is_not_hyperelliptic:=true, simplify_cubic:=true) 
    ...
    */
    ...
    //M:=FindModularForms(k,M,prec);  //prec is put into M`prec here. I need to increase the prec through here for the function to get out of runtime errors.
    ...
    // We now check that enough terms of the q-expansions are computed to impose the vanishing.
    if  &or[ mult[i]/widths[i] ge M`prec/N : i in [1..#mult] ] then xxx:=prec+15; continue; end if;
    // We now construct a Q-basis FF of V.
    C:=[];
    for f in M`F do        
        c:=&cat[ &cat[Eltseq( Coefficient(f[i],j*(N div widths[i])) ) : j in [0..mult[i]-1]] : i in [1..#cusps] ];
  ...
  //  M:=FindModularFormsWithVanishingConditions(M,mult);    //Problem comes up really here. It uses M`prec assigned before but the assigned prec used before as well. So need to change the thing in the above function
    F:=M`F0;
