//Prototype for LLL twist curve.
intrinsic TwistCurveLLL(M::Rec, xi::HomGrp, K::Fld : redcub:=false) -> SeqEnum[RngMPolElt], AlgMatElt, RngIntElt
{
    Input:
    ...
    if Dimension(VV2) ne 0 then
        ...
        preLLLcoef2:=[];
        for f in I2G do
            Append(~preLLLcoef2,[MonomialCoefficient(Pol!f,m): m in mon2]);
        end for;
        LLLMatrix:=[];
        for i in [1..#preLLLcoef2] do
            LLLMatrix:=LLLMatrix cat [LCM([Denominator(k): k in preLLLcoef2[i]])*preLLLcoef2[i][t]:t in [1..#preLLLcoef2[i]]];
        end for;
        if not LLLMatrix eq [] then
            LLLMat:=Matrix(Rationals(),#preLLLcoef2,#preLLLcoef2[1],LLLMatrix);
            LLLMat:=LLL(LLLMat);
        end if;
