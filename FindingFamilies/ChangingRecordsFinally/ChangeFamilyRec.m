...
for k in Keys(FAM) do
    ...
    if assigned FAM[k]`H then FAM1[k]`H:=FAM[k]`H; else 
        FAM1[k]`B`SL:=true;
        H:=FindSpecialSubgroup(FAM1[k]`calG,FAM1[k]`B);
        if GL2DeterminantIndex(H) eq 1 then 
            if GL2Level(H) eq 1 then FAM1[k]`H:=GL2Project(H,2); else            
            FAM1[k]`H:=GL2Project(H,GL2Level(H));
            FAM1[k]`index:=GL2Index(FAM1[k]`H);
             end if;
        end if;
    end if;
...
I:=Open("/homes/ek693/Main/FindingFamilies/ChangingRecordsFinally/Final024.dat", "w");
