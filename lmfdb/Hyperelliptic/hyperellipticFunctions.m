...
function HyperellipticModelFromGroup(G,canM,gonMAT : i:=1, prec0:=0)
	cyclevel:=LCM([canM`N,#BaseRing(G)]);
        //cyctop<o>:=CyclotomicField(cyclevel);
        cycG:=CyclotomicField(#BaseRing(G));
		potentialprec:=Ceiling(Maximum([(canM`prec[i]*canM`sl2level)/canM`widths[i]: i in [1..#canM`cusps]]));
		if potentialprec lt prec0 then 
		canM`H`SL:=true;
        canM:=IncreaseModularFormPrecision(canM,[prec0:i in [1..canM`vinf]]); end if;//arbitrary choice 25.
